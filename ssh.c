...
static void ssh1_log_outgoing_packet ( Ssh ssh, struct Packet *pkt )
{
    ...
    if ( ( pkt->type == SSH1_CMSG_AUTH_PASSWORD ||
        ...
        if ( pkt->type == SSH1_CMSG_X11_REQUEST_FORWARDING &&
                conf_get_int ( ssh->conf, CONF_logomitpass ) ) {
            /*
...
static void ssh2_log_outgoing_packet ( Ssh ssh, struct Packet *pkt )
{
    ...
    if ( pkt->type == SSH2_MSG_USERAUTH_REQUEST &&
        ...
        if ( ssh->pkt_actx == SSH2_PKTCTX_KBDINTER &&
            ...
            if ( pkt->type == SSH2_MSG_CHANNEL_REQUEST &&
                    conf_get_int ( ssh->conf, CONF_logomitpass ) ) {
                /*
                 * If this is an X forwarding request packet, blank the fake
                 * auth data.
    ...
    while ( 1 ) {
        while ( bufchain_size ( &ssh->queued_incoming_data ) > 0 || datalen > 0 ) {
            if ( ssh->frozen ) {
                ...
                if ( bufchain_size ( &ssh->queued_incoming_data ) > 0 ) {
                           ...
                           struct Packet *pktin )
{
     ...
     */
    {
        ...
        if ( s->dlgret == 0 ) {        /* did not match */
            ...
            if ( s->dlgret < 0 ) { /* none configured; use standard handling */
                ssh_set_frozen ( ssh, 1 );
    ...
    while ( pktin->type == SSH1_SMSG_FAILURE ) {
        ...
        if ( s->pwpkt_type == SSH1_CMSG_AUTH_PASSWORD ) {
            ...
            if ( ! ( ssh->remote_bugs & BUG_CHOKES_ON_SSH1_IGNORE ) &&
                ...
                if ( ! ( ssh->remote_bugs & BUG_NEEDS_SSH1_PLAIN_PASSWORD ) ) {
                    /*
            ...
            if ( epf->remote ) {
                ...
                if ( ssh->version == 1 ) {
                    ...
                    if ( epf->saddr ) {
                        ...
                        if ( conf_get_int ( conf, CONF_rport_acceptall ) ) {
     ...
     */
    {
        ...
        for ( i = 0; i < lenof ( compressions ) + 1; i++ ) {
            ...
            if ( in_commasep_string ( c->name, str, len ) ) {
                ...
                if ( in_commasep_string ( c->delayed_name, str, len ) ) {
